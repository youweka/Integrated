...
from modules.schemas import (
    ...
    CategoryCount,
    FileCategorizationResponse, # Import FileCategorizationResponse explicitly
    TransactionVisualizationRequest
)
from modules.acu_parser.zip_extractor import extract_from_directory
...
async def process_zip_file(
    ...
    try:
        ...
        # Step 3.6: EXTRACT ACU FILES AUTOMATICALLY
        print(f"🔧 Searching for ACU files (jdd*, x3*) in extracted path...")
        print(f"  🔍 ACU search base path: {extract_path}")
        
        # Debug: List some files in the extraction path to verify content
        debug_files_found = []
        for f in extract_path.rglob('*'):
            if f.is_file():
                debug_files_found.append(str(f.relative_to(extract_path)))
            if len(debug_files_found) >= 10: # Limit to first 10 for brevity
                break
        print(f"  🔍 First 10 files (relative to base) in extracted path: {debug_files_found}")

        acu_logs = []
        acu_files = extract_from_directory(
            extract_path, 
            acu_logs, 
            target_prefixes=('jdd', 'x3')
        )
        print(f"  🔍 `extract_from_directory` returned {len(acu_files)} potential ACU files.")
        if acu_files:
            # Add ACU files to the categories for display
            xml_files = [k for k in acu_files if not k.startswith('__xsd__')]
            file_categories['acu_xml_files'] = xml_files
            print(f"  ✓ Found {len(xml_files)} ACU XML files.")
        else:
            print(f"  ⚠️ No ACU files found by `extract_from_directory` for prefixes ('jdd', 'x3').")
            if acu_logs:
                print(f"  ⚠️ ACU extraction logs: {acu_logs}")
        ...
        session_service.update_session(CURRENT_SESSION_ID, 'acu_extraction_logs', acu_logs) # Store logs in session
        # Debug output
        ...
        result.acu_extraction_logs = acu_logs # Add ACU logs to the response
...
from modules.acu_parser.zip_extractor import extract_from_zip_bytes, extract_from_directory
from modules.acu_parser.xml_parser_logic import parse_xml_to_dataframe

@router.post("/extract-files/")
async def extract_acu_files(file: UploadFile = File(...)):
    """
    Extract XML and XSD files from uploaded ZIP archive for ACU parsing
    """
    try:
        print(f"📦 Extracting ACU files from: {file.filename}")
        ...
        # Extract files
        logs = []
        extracted_files = extract_from_zip_bytes(
            zip_content, 
            logs, 
            target_prefixes=('jdd', 'x3')
        )
        ...
        if not extracted_files:
            raise HTTPException(
                status_code=400,
                detail="No ACU configuration files (jdd*, x3*) found in the archive"
            )
        ...
        return {
            ...
            "logs": logs,
            "file_count": len([k for k in extracted_files if not k.startswith('__xsd__')]),
            "xsd_count": len([k for k in extracted_files if k.startswith('__xsd__')])
        }
...
class ParseFilesRequest(BaseModel):
    """Request model for parsing ACU files"""
    filename: str
    xml_content: str
...
async def parse_acu_files(files: List[ParseFilesRequest]):
    """
    Parse ACU XML files with optional XSD documentation
    """
    try:
        print(f"\n📝 Parsing {len(files)} ACU file(s)")
        ...
        for file_data in files:
            ...
            # Parse the XML file (no debug parameter)
            df = parse_xml_to_dataframe(
                file_data.xml_content,
                file_data.filename,
                file_data.xsd_content
            )
            ...
            if not df.empty:
                ...
                # Count how many have documentation
                with_docs = len([r for r in records if r.get('Details')])
                print(f"  ✓ Extracted {len(records)} parameters ({with_docs} with documentation)")
            else:
                print(f"  ⚠️ No data extracted from {file_data.filename}")
        
        if not all_records:
            return {
                "data": [],
                "count": 0,
                "message": "No data could be extracted from the selected files"
            }
        
        # Summary stats
        total_with_docs = len([r for r in all_records if r.get('Details')])
        print(f"\n✅ Total: {len(all_records)} parameters ({total_with_docs} with documentation)")
        
        return {
            "data": all_records,
            "count": len(all_records),
            "documented_count": total_with_docs,
            "message": f"Successfully parsed {len(files)} file(s)"
        }
...
class PathRequest(BaseModel):
    path: str
    prefixes: List[str] = ['jdd', 'x3'] # Default prefixes

@router.post("/extract-acu-from-path/")
async def extract_acu_from_path(request: PathRequest):
    """
    Extracts ACU files from a given directory path, searching recursively.
    Accepts a list of file prefixes to search for.
    """
    try:
        ...
        target_prefixes = tuple(request.prefixes)
        print(f"🔍 Starting ACU extraction from path: {base_path} for prefixes: {target_prefixes}")
        logs = []
        
        # Use the updated extractor function
        all_files = extract_from_directory(
            base_path, 
            logs, 
        ...
        if not all_files:
            raise HTTPException(
                status_code=404,
                detail=f"No ACU configuration files with prefixes {target_prefixes} found in the specified path or its sub-archives."
            )

        xml_count = len([k for k in all_files if not k.startswith('__xsd__')])
        xsd_count = len([k for k in all_files if k.startswith('__xsd__')])
        
        print(f"✅ Extraction complete: {xml_count} XML, {xsd_count} XSD files found.")
        ...
        raise HTTPException(
            status_code=500,
            detail=f"Extraction from path failed: {str(e)}"
        )