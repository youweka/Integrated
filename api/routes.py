...
async def extract_acu_from_path(request: PathRequest):
    ...
    This is the new consolidated endpoint for ACU extraction.
    """
    try:
        ...
        print(f"🔍 Starting ACU extraction from path: {base_path}")
        logs = []
        ...
        if not all_files:
            raise HTTPException(
                status_code=404,
                detail="No ACU configuration files (jdd*, x3*) found in the specified path or its sub-archives."
            )
