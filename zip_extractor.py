...
def extract_from_zip_bytes(zip_bytes: bytes, logs: List[str], target_prefixes=('jdd', 'x3')) -> Dict[str, str]:
    ...
    data = zip_bytes
    size = len(data)
    pos = 0 # Current position in the byte stream
    ...
    while True:
        ...
        if pos == -1: # No more Central Directory signatures found
            logs.append("No more Central Directory signatures found. Breaking.")
            break
        try:
            if pos + cd_struct_size > size: # Check if header itself is truncated
            ...
            if fname_end > size: # Check if filename is truncated
            ...
            logs.append(f"  Processing entry: {norm_fname} (base: {base_lower})")
            # Check if this is a target XML file
            is_target_xml = base_lower.endswith('.xml') and any(base_lower.startswith(p.lower()) for p in target_prefixes
            )
            
            # Check if this is a target XSD file (matching same prefixes)
            is_target_xsd = base_lower.endswith('.xsd') and any(base_lower.startswith(p.lower()) for p in target_prefixes
            )
            
            # Check for nested zip files
            is_nested_zip = base_lower.endswith('.zip')

            if is_target_xml or is_target_xsd or is_nested_zip: # If it's a file we care about
                ...
                if data[lh_off:lh_off + 4] != LH_SIG:
                    search_from = max(0, lh_off - 8) # Search a small window around expected offset
                ...
                try:
                    if comp_method == 0: # Stored (uncompressed)
                ...
                if is_nested_zip: # Recursive call for nested ZIPs
                    logs.append(f"Found nested zip entry {norm_fname}, attempting in-memory extraction.")
                    nested_logs = [] # Use a separate log list for nested calls to keep main log cleaner
                    nested_found = extract_from_zip_bytes(file_bytes, nested_logs, target_prefixes=target_prefixes)
                    logs.extend([f"      (Nested) {l}" for l in nested_logs]) # Indent nested logs
                    ...
                    while key in found:
                        ...
                        i += 1
                    found[key] = text
                    logs.append(f"Extracted XML: {norm_fname} (size={len(text)} chars) -> {key}")
            else:
                logs.append(f"  No match for {norm_fname}. Skipping.")
    ...
    xml_count = sum(1 for k in found if not k.startswith('__xsd__')) # Count actual XMLs
...
def extract_from_directory(base_path: Path, logs: List[str], target_prefixes=('jdd', 'x3')) -> Dict[str, str]:
    ...
    """
    all_found_files: Dict[str, str] = {} # Accumulator for all found files
        ...
        for zip_file in zip_files:
            try:
                with open(zip_file, 'rb') as f:
                    zip_bytes = f.read()
                logs.append(f"  Processing nested ZIP: {zip_file.relative_to(base_path)}")
    ...
    for file_path in base_path.rglob("*"):
        if file_path.is_file():
            file_name = file_path.name # Basename of the file
            file_name_lower = file_name.lower()
            logs.append(f"  Checking direct file: {file_path.relative_to(base_path)} (lower: {file_name_lower})")
            ...
            if is_target_file:
                try:
                    ...
                    if key not in all_found_files:
                        all_found_files[key] = content # Add to results
                        logs.append(f"    Found direct file: {file_path.relative_to(base_path)} -> {key}")
                    else:
                        logs.append(f"    Direct file {file_path.relative_to(base_path)} already found (key: {key}). Skipping duplicate.")
                except Exception as e:
                    logs.append(f"  ERROR reading direct file {file_name}: {e}")
            else:
                logs.append(f"    No match for direct file {file_path.relative_to(base_path)}. Skipping.")
