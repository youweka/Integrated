...
def render_individual_transaction_analysis():
    ...
    try:
        ...
        with st.expander("🐛 Debug Information"):
            st.code(traceback.format_exc())
def render_acu_single_parse(): # This function remains as is from previous changes
    """Render ACU Parser for single ZIP archive - replicating standalone tool logic"""
    ...
    st.markdown("#### Step 1: Upload ZIP Archive")
    uploaded_file = st.file_uploader(
        "Select a ZIP file containing ACU configurations",
        type=['zip'],
        key="acu_zip_uploader"
    )

    if uploaded_file:
        if st.button("Process Uploaded Archive", type="primary", key="acu_process_btn"):
            with st.spinner("Extracting ACU files from archive..."):
                try:
                    files_payload = {'file': (uploaded_file.name, uploaded_file.getvalue(), 'application/zip')}
                    response = requests.post(f"{API_BASE_URL}/extract-files/", files=files_payload, timeout=120)

                    if response.status_code == 200:
                        result = response.json()
                        st.session_state.acu_extracted_files = result.get("files", {})
                        st.session_state.acu_parsed_df = None # Reset previous parse results
                        st.success(f"✓ Extracted {result.get('file_count', 0)} XML and {result.get('xsd_count', 0)} XSD files.")
                        st.rerun()
                    else:
    ...
    if 'acu_extracted_files' in st.session_state:
        extracted_files = st.session_state.get('acu_extracted_files')
        ...
        if xml_files:
            st.markdown("---")
            st.markdown("#### Step 2: Select File to Parse")
            ...
            if st.button("🚀 Parse Selected File", key="acu_parse_btn"):
    ...
    if 'acu_parsed_df' in st.session_state and not st.session_state['acu_parsed_df'].empty:
        ...
        st.markdown("---")
        st.markdown("#### Step 3: Analyze Parsed Data")
        ...
        st.download_button(
            label="📥 Download as CSV",
            data=csv,
            file_name=f"acu_config_{Path(st.session_state.get('acu_zip_uploader').name).stem if st.session_state.get('acu_zip_uploader') else 'export'}.csv",
            mime="text/csv"
        )

def render_acu_compare():
    """Render ACU Parser for comparing two ZIP archives - with independent ZIP uploaders."""
    ...
    if comp_data.get('files1'):
        ...
        if st.button("Replace Source A", key="acu_replace_a"):
            comp_data['zip1_name'] = None
            comp_data['files1'] = None
            comp_data['files1_all'] = None
            st.rerun()
    else:
        zip1 = st.file_uploader("Upload First ZIP for comparison", type="zip", key="acu_zip1")
        if zip1 and st.button("Process Source A", key="acu_process_a"):
            with st.spinner(f"Extracting ACU files from {zip1.name}..."):
                try:
                    # Use the dedicated backend endpoint for consistent extraction logic
                    response = requests.post(
                        f"{API_BASE_URL}/extract-acu-from-path/",
                        files={'file': (zip1.name, zip1.getvalue(), 'application/zip')},
                        timeout=180
                    )
                    ...
                    if response.status_code == 200:
                        ...
                        comp_data['zip1_name'] = zip1.name
                        comp_data['files1'] = {k: v for k, v in all_acu_files.items() if not k.startswith('__xsd__')}
                        comp_data['files1_all'] = all_acu_files  # Include XSD files
